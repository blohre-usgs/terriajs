            ...
            errorEvent.addEventListener(function(tileProviderError) {
                ...
                if (catalogItem.isShown) {
                    catalogItem.terria.error.raiseEvent(new TerriaError({
                        sender: catalogItem,
                        title: 'Error accessing catalog item',
                        message: '\
An error occurred while attempting to download tiles for catalog item ' + catalogItem.name + '.  This may indicate that there is a \
problem with your internet connection, that the catalog item is temporarily unavailable, or that the catalog item \
is invalid.  The catalog item has been hidden from the map.  You may re-show it in the Now Viewing panel to try again.'
                    }));
