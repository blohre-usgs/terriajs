...
function isEnabledChanged(catalogItem) {
    ...
    if (catalogItem.isEnabled) {
        ...
        checkAndHandleMutuallyExclusiveItems(catalogItem);
    } else {
...
function checkAndHandleMutuallyExclusiveItems(catalogItem) {
    var catalogGroups = catalogItem.terria.catalog.group.items;
    catalogGroups.forEach(function(catalogGroup) {
        if(!catalogGroup.mutuallyExclusiveItems) {
            return;
        }
        var containsCatalogItem = false;
        var itemsToTurnOff = [];
        var groupItems = catalogGroup.items;
        groupItems.forEach(function(groupItem) {
            if(groupItem === catalogItem) {
                containsCatalogItem = true;
                return;
            }
            if(groupItem.isEnabled) {
                itemsToTurnOff.push(groupItem);
            }
        });
        if(containsCatalogItem) {
            itemsToTurnOff.forEach(function(item) {
                item.toggleEnabled();
            });
        }
    });
